...
- 「なぜそうするか(事故例つき)」の解説は [概念解説集](laravel-concepts.md) に、
  「どのステップで登場するか」は [開発トレースガイド](dev-walkthrough.md) にあります。
  この文書は**構文の橋渡し**に徹し、詳細はそちらへリンクします
...
- 実例: [`TodoPolicy`](../../app/Policies/TodoPolicy.php) の全メソッド、
...
- 実例: [`todos/show.blade.php`](../../resources/views/todos/show.blade.php) の期限日表示
...
- 実例: [`TodoController::index()`](../../app/Http/Controllers/TodoController.php) の絞り込みクロージャ、
...
- 実例: [`StoreTodoRequest::rules()`](../../app/Http/Requests/StoreTodoRequest.php) の返り値
...
- 実例: [`Todo`](../../app/Models/Todo.php) の `use HasFactory;`、
...
- 実例: [`User`](../../app/Models/User.php) の `#[Fillable]` / `#[Hidden]`
