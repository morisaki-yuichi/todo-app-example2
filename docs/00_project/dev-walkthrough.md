...
| US-13 自分のTODOだけ(認可) | 8-1/8-2([cc79094](https://github.com/morisaki-yuichi/todo-app-example2/commit/cc79094))、8-3 FormRequest([d9fd341](https://github.com/morisaki-yuichi/todo-app-example2/commit/d9fd341)) | [#9](https://github.com/morisaki-yuichi/todo-app-example2/pull/9) |
...
| わざと失敗実験: 認可を外すと他人のTODOが200で見える(戻すと403) | このガイドの [実験8-A](#実験8-a-認可の効き目を確かめるそして守りを外して見る) |
| user_id導入で既存Featureテスト12本が赤(所有者違い)→ for()で更新 | [ステップ8-2](#ステップ8-2-policyで他人のtodoを403にする) |
...
# スプリント8: 認可 + 負債返済 + 教材最終化(第2部の完了)

**ゴール**: TODOに所有者を持たせ、自分のTODOだけを扱える(他人のは403)ようにし、
バリデーション重複をFormRequestで回収して、第2部の教材を完成させる
(計画: [スプリント8バックログ](../08_sprint8/sprint-backlog.md) / PR: [#9](https://github.com/morisaki-yuichi/todo-app-example2/pull/9))

【概念】[認可とPolicy](laravel-concepts.md#40-認可とpolicy) /
[リレーションとデータ移行](laravel-concepts.md#41-リレーションと既存データの移行) /
[FormRequest](laravel-concepts.md#42-formrequestとバリデーションの集約)

## ステップ8-1: user_idの追加とデータ移行(T-16)

- 差分: [GitHubで見る](https://github.com/morisaki-yuichi/todo-app-example2/commit/cc79094) / ローカル: `git show cc79094`

### これから何を・なぜやるか

TODOに持ち主(user_id)を持たせます。**既に12件のTODOが本番相当のDBにある**ので、
いきなりNOT NULLの列を足すと既存行が値を持てず失敗します。3段階で移行します:

1. `user_id` を**nullableで追加**(既存行はNULLで生き残る)
2. 既存行を先頭ユーザー(=デモ)に**紐付ける**(データ移行)
3. 全行が値を持ったので **NOT NULL + 外部キー制約**を付ける

これを**1つのマイグレーションのup()にまとめる**のが勘所。「列追加」と「移行」を
別マイグレーションにすると、途中の状態(NULLがある)が他環境に伝わる恐れがあります。

### 足場の作り方

| ファイル | 作り方 |
|---|---|
| `database/migrations/XXXX_add_user_id_to_todos_table.php` | **ジェネレータ**(`--table=todos`)後、**手で編集**(3段階) |
| `app/Models/Todo.php` / `User.php` | **手で編集**(belongsTo / hasMany、$fillableにuser_id) |
| `database/factories/TodoFactory.php` | **手で編集**(user_idの既定=User::factory()) |
| `database/seeders/TodoSeeder.php` | **手で編集**(2ユーザー・所有者つき) |

### 動作確認(CLI): データが生き残る移行

```bash
./vendor/bin/sail artisan tinker --execute="echo App\Models\Todo::count();"   # 移行前 12
./vendor/bin/sail artisan migrate
./vendor/bin/sail artisan tinker --execute="
DB::table('todos')->selectRaw('user_id, count(*) c')->groupBy('user_id')->get()->each(fn(\$r)=>print(\$r->user_id.'→'.\$r->c.'件'.PHP_EOL));
"   # => 1→12件(全部デモに紐付いた)
./vendor/bin/sail artisan migrate:rollback --step=1   # データは残り列だけ消える
./vendor/bin/sail artisan migrate
```

### ここでコミット(認可とセット)

user_id追加・リレーション・一覧スコープ・Policy・テスト更新を1コミットに。
「所有権を導入する」という1つの意味だからです(ステップ8-2の内容も含む)。

## ステップ8-2: Policyで他人のTODOを403にする

- 差分: 上と同じコミット([cc79094](https://github.com/morisaki-yuichi/todo-app-example2/commit/cc79094))

### これから何を・なぜやるか

「見る・更新・削除」は持ち主だけに許します。判定を各コントローラにバラ書きすると
書き忘れが穴になるので、**Policy**(認可ルールを1箇所に集めたクラス)に寄せます。

### 足場の作り方

| ファイル | 作り方 |
|---|---|
| `app/Policies/TodoPolicy.php` | **ジェネレータ**(`make:policy --model=Todo`)後、**手で編集** |
| `app/Http/Controllers/TodoController.php` | **手で編集**(各操作に `$this->authorize(...)`、indexを自分の範囲に) |
| `app/Http/Controllers/Controller.php` | **手で編集**(`use AuthorizesRequests`) |

- Policyは**命名規約でLaravelが自動発見**(TodoPolicy ↔ Todo)。手動登録は不要
- `$this->authorize('view', $todo)` は、Policyがfalseを返すと**自動で403**
- 一覧は「そもそも他人のを取得しない」= `$request->user()->todos()` でスコープ
- Laravel 11+は基底Controllerに `AuthorizesRequests` が無いので**手で取り込む**
  (無いと `$this->authorize` が「メソッドが無い」で落ちる)

### 実験8-A: 認可の効き目を確かめる(そして守りを外して見る)

**予想**: 他人のTODOのURLを直叩きすると403。authorizeを外すと200で見えてしまう。

```bash
# demoでログイン後、別ユーザー(other)のTODOのIDを直叩き
curl -s -b $JAR -o /dev/null -w "%{http_code}\n" http://localhost:8080/todos/<他人のID>  # => 403
```

- **結果**: 自分の詳細=200、他人の詳細/編集/削除確認=すべて403。一覧に他人のは出ない
- **守りを外す実演**: `show()` の `$this->authorize('view', $todo)` を一時的に
  コメントアウトすると、同じURLが**200になり他人のTODOが丸見え**になりました
  (確認後すぐ戻す。この実験はコミットしない)。「認可は1行の門番。外すと即穴」を体感

### 認可のテストは本人/他人をペアで(T-17)

`TodoAuthorizationTest` に「本人はできる/他人は403」を各操作でペア実装:

```php
$this->actingAs($this->owner)->get("/todos/{$todo->id}")->assertOk();
$this->actingAs($this->stranger)->get("/todos/{$todo->id}")->assertForbidden();
```

**片側だけのテストは危険**: 「本人ができる」だけ確認して「他人ができない」を
書かないと、認可が壊れていても気づけません。403の確認こそ本体です。

> **既存テストの更新**: user_id導入で、各FeatureテストのTODOが「ログインユーザーとは
> 別の持ち主」になり12本が403等で赤化。`Todo::factory()->for($this->user)` で
> ログインユーザー所有にして緑に戻しました(認可の副作用としての予想どおりの赤)。

## ステップ8-3: FormRequestで負債を返す(T-18)

- 差分: [GitHubで見る](https://github.com/morisaki-yuichi/todo-app-example2/commit/d9fd341) / ローカル: `git show d9fd341`

### これから何を・なぜやるか

第1部(スプリント4)で**あえて残した**バリデーションの重複(store と update)を、
いま回収します。テストが39本ある**今なら安全にリファクタできる** —
「振る舞いを変えないこと」をテストが保証してくれるからです。

### T-18: 緑 → 変更 → 緑 のリズム

```bash
./vendor/bin/sail test        # まず緑(39)を確認 ← リファクタ前の状態を記録
# StoreTodoRequest / UpdateTodoRequest を作り、コントローラをそれに差し替え
./vendor/bin/sail test        # 再び緑(39)← 振る舞いが変わっていない証拠
```

- リファクタリングの定義は「**外から見た振る舞いを変えずに**内部構造を改善する」。
  テストが緑のままであることが、その「振る舞い不変」の担保になります
- StoreとUpdateは同一ルールだが**別クラス**にしました。「将来別ルールになりうる」
  ため。第1部の[DRYの議論](laravel-concepts.md#29-dry)の続き — 共通化は常に正義ではなく、
  重複のコストと将来の分岐可能性を天秤にかける

### ここでコミット

```bash
git add -A && git commit -m "refactor: バリデーションをFormRequestに集約(負債返済)"
```

- **種別の理由**: 振る舞いを変えないので `feat` ではなく `refactor`

## ステップ8-4: 教材最終化(v2)

第2部の対応マップ・コミット外の出来事一覧・READMEを最終化します(このステップの
コミットはドキュメントのみ)。プロジェクト全体の完了は
[スプリント8レビュー記録](../08_sprint8/sprint-review.md)で宣言します。

---

## スプリント8の振り返り課題(写経者向け)

回答例は[スプリント8レトロスペクティブ](../08_sprint8/sprint-retrospective.md)にあります。

1. 認証(スプリント7)と認可(スプリント8)の違いを、302と403の使い分けで説明せよ
2. 稼働中テーブルにNOT NULLの列を足すとき、なぜ3段階(nullable追加→移行→NOT NULL化)
   にするのか
3. 「テストがあるとリファクタリングが安全になる」とはどういう意味か。T-18の
   「緑→変更→緑」と結びつけて説明せよ
