...
| [PHP・Laravelクイックキャッチアップ](docs/00_project/php-for-polyglots.md) | **写経を始める前に**。Python/JavaScript経験者がコードを読めるようになる構文の対応表(自己完結) |
| [演習編](docs/00_project/exercises.md) | 考えて書く→模範解答・解説で答え合わせ。テーマA=写経前でも可、テーマB以降=写経後(サンプル版) |
...
> **ポートについて**: このプロジェクトは `.env` の `APP_PORT=8080` / `VITE_PORT=5180` /
> `FORWARD_DB_PORT=3306`(MySQLをホストに公開するポート)でホスト側ポートを明示しています。
> 手元で既に使用中の場合は、`.env` の値を空いているポートに変更してください
> (空き確認: `ss -tlnp | grep 8080`)。**別のMySQLやSailプロジェクトが動いていると
> 3306が衝突しやすい**ので注意。`COMPOSE_PROJECT_NAME` は、コンテナ名が
> 他プロジェクトと衝突しないための設定です。
...
デモユーザー(`demo@example.com` / パスワード `password`)と、
そのユーザーのサンプルTODOが入ります。

### 6. ブラウザで確認する

http://localhost:8080 を開くとログイン画面(`/login`)に誘導されます。
上のデモユーザーでログインするか、「新規登録」からアカウントを作ると、
自分のTODO一覧が表示されます。

> このアプリはログイン必須で、**各ユーザーは自分のTODOだけ**を見られます
> (他人のTODOにアクセスすると403)。認証・認可の実装過程は
> [開発トレースガイド](docs/00_project/dev-walkthrough.md)のスプリント7〜8で追えます。
